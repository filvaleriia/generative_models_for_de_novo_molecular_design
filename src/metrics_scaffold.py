...
from src.path_utils import data_subdir, scaffold_results_dir


VALID_CLUSTER_TYPES = {"dis", "sim"}
VALID_SCAFFOLD_TYPES = {"csk", "murcko"}
...
def scaffold_cache_path(filepath: str | Path, scaffold_type: str) -> Path:
    """Store scaffold cache next to the original input/output file."""
    path = Path(filepath)
    return path.with_suffix(path.suffix + f".{scaffold_type}.scaffolds.npy")


def load_scaffold_cache(filepath: str | Path, scaffold_type: str) -> Optional[pd.DataFrame]:
    """Load cached scaffolds if they exist."""
    cache_path = scaffold_cache_path(filepath, scaffold_type)
    ...
    return pd.DataFrame(cached).dropna()


def save_scaffold_cache(filepath: str | Path, scaffold_type: str, scaffolds_df: pd.DataFrame) -> Path:
    """Save scaffolds next to the original input/output file."""
    cache_path = scaffold_cache_path(filepath, scaffold_type)
    arr = scaffolds_df[0].to_numpy(dtype=object) if not scaffolds_df.empty else np.asarray([], dtype=object)
    np.save(cache_path, arr, allow_pickle=True)
    return cache_path
...
class Metrics_scaffold:
    ...
    def __init__(
        ...
        if type_scaffold not in VALID_SCAFFOLD_TYPES:
            raise ValueError(
                f"Unsupported scaffold type '{type_scaffold}'. Use one of {sorted(VALID_SCAFFOLD_TYPES)}."
            )
    ...
    def _get_output_dir(self) -> str:
        ...
        return str(
            scaffold_results_dir(
                self.data_folder,
                self.receptor,
                f"{self.type_scaffold}_scaffolds",
                self.type_cluster,
                self.generator_name,
            )
        )
    ...
    def load(self, filepath_output_set: str, filepath_recall_set: str):
        ...
        self.filepath_output_set = filepath_output_set
        self.filepath_recall_set = filepath_recall_set
    ...
    def convert_to_scaffolds(self, use_cache: bool = True):
        """Convert SMILES to scaffolds using multiprocessing, with optional cache."""
        print(f"\nConverting to {self.type_scaffold} scaffolds...")

        # Convert recall set
        print("  Processing recall set...")
        self.recall_set_scaffolds = self._convert_or_load_scaffolds(
            smiles_df=self.recall_set,
            source_path=self.filepath_recall_set,
            dataset_label="Recall",
            use_cache=use_cache,
        )
        ...
        self.output_set_scaffolds = self._convert_or_load_scaffolds(
            smiles_df=self.output_set,
            source_path=self.filepath_output_set,
            dataset_label="Output",
            use_cache=use_cache,
        )
    ...
    def _convert_or_load_scaffolds(
        self,
        ...
        if use_cache:
            cached = load_scaffold_cache(source_path, self.type_scaffold)
            if cached is not None:
                print(f"    {dataset_label} cache hit: {scaffold_cache_path(source_path, self.type_scaffold)}")
                return cached

        with Pool(processes=self.ncpus) as pool:
            results = pool.starmap(
                convert_to_scaffold,
                [(smiles, self.type_scaffold) for smiles in smiles_df[0]]
            )

        scaffolds_df = pd.DataFrame(results).dropna()
        if use_cache:
            cache_path = save_scaffold_cache(source_path, self.type_scaffold, scaffolds_df)
            print(f"    {dataset_label} cache saved: {cache_path}")
        return scaffolds_df

    def calculate_metrics(self, use_cache: bool = True):
        """Calculate similarity metrics between output and recall sets."""
        print("\nCalculating metrics...")
        
        # Convert to scaffolds
        self.convert_to_scaffolds(use_cache=use_cache)
    ...
    def calculate(self, cluster_range: range = range(5), use_cache: bool = True):
        """
        ...
        for number in cluster_range:
            ...
            if os.path.exists(output_file_path):
                ...
                self.calculate_metrics(use_cache=use_cache)
                self.save_results()
...
MetricsScaffold = Metrics_scaffold
