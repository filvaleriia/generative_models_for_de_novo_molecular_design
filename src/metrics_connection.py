...
def _scaffold_results_dir(data_folder, receptor, type_scaffold, type_cluster) -> Path:
    if hasattr(path_utils, "scaffold_results_dir"):
        return path_utils.scaffold_results_dir(
            data_folder,
            receptor,
            f"{type_scaffold}_scaffolds",
            type_cluster,
        )
...
def connect_mean_value(type_cluster, type_scaffold, generators_name_list, receptor, subset='', data_folder=''):
    ...
    # Define path to data
    link = _scaffold_results_dir(data_folder, receptor, type_scaffold, type_cluster)
    
    # List to store paths of mean value files
    link_mean = []
    for generator in generators_name_list:
        link_mean.append(link / generator / f"{generator}_mean_{type_scaffold}_{type_cluster}.csv")
    ...
    output_path = link / f"mean_{type_scaffold}_{type_cluster}{subset}.csv"
...
def connect_mean_value_normalized(type_cluster, type_scaffold, generators_name_list, receptor, subset, data_folder = ''):
    ...
    # List to store paths of mean value files
    link_mean = [link / generator / f"{generator}_mean_{type_scaffold}_{type_cluster}.csv" for generator in generators_name_list]
    ...
    output_path = link / f"mean_{type_scaffold}_{type_cluster}{subset}_norm_min_max.csv"
    output_path.parent.mkdir(parents=True, exist_ok=True)
    df.to_csv(output_path, index=False)
...
def main():
    ...
    out_xlsx = str(
        _scaffold_results_dir(args.data_folder, args.receptor, args.type_scaffold, args.type_cluster)
        / f"paper_table_{args.receptor}_{args.type_scaffold}{args.subset}.xlsx"
    )
